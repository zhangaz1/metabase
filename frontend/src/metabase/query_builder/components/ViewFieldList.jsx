    ...
    return (
      <Box mx={2}>
        <Box mb={1}>
          <input
            type="text"
            placeholder="Search for a field to filter on"
            className="input full"
          />
        </Box>
        {this.state.sections.map(s => {
          return (
            <Box mb={2} bg="white" className="bordered rounded shadowed">
              <Flex
                align="center"
                p={2}
                className="border-bottom hover-parent hover--visibility"
              >
                <h3>{s.name}</h3>
                <Icon
                  name="chevronup"
                  size={12}
                  ml="auto"
                  className="hover-child cursor-pointer text-brand-hover"
                />
              </Flex>
              <Box />
              <Box px={1}>
                {s.items.map(i => (
                  ...
                  >
                    <Box mr={1}>{this.renderItemIcon(i)}</Box>
                    <h4>{i.name}</h4>
                  </Flex>
                ))}
              </Box>
